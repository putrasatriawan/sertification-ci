...
{
    $this->db->select("users.*, roles.name as role_name, unit_kerja.name as unit_kerja_name")->from("users");
    $this->db->join("users_roles", "users.id = users_roles.user_id");
    $this->db->join('unit_kerja', 'users.unit_kerja = unit_kerja.id', 'left');
    $this->db->join("roles", "roles.id = users_roles.role_id");
    ...
    if (!empty($search)) {
        foreach ($search as $key => $value) {
            if ($key === 'role_name') { // Ubah 'roles.name' menjadi 'role_name'
                $this->db->like('roles.name', $value); // Ubah $key menjadi 'roles.name'
            } else {
